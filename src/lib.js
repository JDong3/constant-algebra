  ...
  mm: {
    ...
    mul: (matrix, matrix2, i=0, res=EMPTY_LIST) => {
      if (i >= matrix.size) {
        ...
        const update = res.push(lib.mm.subMul(matrix.get(i), matrix2))
        return lib.mm.mul(matrix, matrix2, i+1, update)
      }
    },
    subMul: (vector, matrix, i=0, res=EMPTY_LIST) => {
      if (i >= matrix.get(0).size) {
        ...
        const update = res.push(lib.vn.dot(vector, lib.mv.column(matrix, i)))
        return lib.mm.subMul(vector, matrix, i+1, update)
      }
    },
    ...
    rowAdd: (matrix, row1, row2, n=1) => (
      matrix.set(row1, lib.mm.rowAfterAdding(matrix, row1, row2, n))
    ),
